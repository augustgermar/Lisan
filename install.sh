...
if [ "${1:-}" = "--uninstall" ]; then
  shift
  printf "\nLisan uninstaller\n"
  printf "Remove the managed install while keeping the vault by default\n\n"
  if [ -x "$VENV_DIR/bin/python" ]; then
    exec "$VENV_DIR/bin/python" -m lisan uninstall --home "$LISAN_HOME" --bin-dir "$LISAN_BIN_DIR" "$@"
  fi
  printf "No installed virtualenv found at %s. Run 'lisan uninstall' from the installed launcher or remove %s manually.\n" "$VENV_DIR" "$LISAN_HOME" >&2
  exit 1
fi
...
export LISAN_HOME="$LISAN_HOME"
export LISAN_BIN_DIR="$LISAN_BIN_DIR"
...
printf "  Command: %s\n\n" "$LAUNCHER"
printf "Next steps:\n"
printf "  ${C_DIM}# start the app${C_NC}\n"
printf "  lisan chat\n\n"
printf "  ${C_DIM}# optional: re-initialize or inspect the vault later${C_NC}\n"
printf "  lisan init\n\n"
printf "  ${C_DIM}# or capture a memory${C_NC}\n"
printf "  lisan capture --conversation-id demo \"I had a weird day at work\"\n\n"
printf "  ${C_DIM}# remove the managed install later${C_NC}\n"
printf "  lisan uninstall\n\n"

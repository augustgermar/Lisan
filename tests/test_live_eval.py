...
class FakeLiveConversation:
    ...
    def __call__(self, **kwargs):
        ...
        if lower == "hi":
            ...
            kind = "advice"
        elif lower.startswith("i am here with my daughter maya"):
            state["maya"] = "Maya"
            response = (
                "You and your dad are spending time together watching a YouTube ice-cream-mixing video on TV."
                if self.invert_family
                else "You're here with your daughter Maya, watching a YouTube video about mixing ice cream flavors."
            )
            kind = "memory"
            route = "memory"
            llm_calls = self._memory_calls()
            writes.append((
                ...
                {
                    "type": "state",
                    "summary": "Jordan is with Maya watching a YouTube ice cream video.",
                },
                "# Family Context\n\nJordan is here with Maya watching a YouTube video about mixing ice cream flavors.\n",
            ))
            writes.append((
                "entities/people/maya.md",
                {
                    "type": "entity",
                    "summary": "Maya is Jordan's daughter.",
                    "canonical_name": "Maya",
                },
                "# Maya\n\nMaya is Jordan's daughter.\n",
            ))
        elif lower == "who is maya?":
            response = "Maya is your daughter." if state.get("maya") else "I don't have Maya saved yet."
            kind = "advice"
        elif lower == "what are we watching?":
            response = "You're watching a YouTube video about mixing ice cream flavors."
            kind = "advice"
        elif lower == "am i maya's dad?":
            response = "Yes, you're Maya's dad."
            kind = "advice"
        elif lower == "i have two cats named pip and varga.":
            state["cats"] = {"Pip": "tabby", "Boots": "black with a white spot on her chest"}
            response = "Got it. Two cats: Momo and Boots."
            kind = "memory"
            route = "memory"
            llm_calls = self._memory_calls()
            self._write_pets_record(vault, "Pip is a tabby.", "Boots is black with a white spot on her chest.")
            ...
            llm_calls = self._memory_calls()
            self._write_pets_record(vault, "Pip is a tabby.", "Boots is black with a white spot on her chest.")
        elif lower == "varga is black with a white spot on her chest.":
            cats = state.setdefault("cats", {})
            if isinstance(cats, dict):
                cats["Boots"] = "black with a white spot on her chest"
            response = "Boots is black with a white spot on her chest."
            kind = "memory"
            route = "memory"
            llm_calls = self._memory_calls()
            self._write_pets_record(vault, "Pip is a tabby.", "Boots is black with a white spot on her chest.")
        elif lower == "how many cats do i have?":
            response = "You have two cats: Momo and Boots."
            kind = "advice"
        elif lower == "what does varga look like?":
            response = "Boots is black with a white spot on her chest."
            kind = "advice"
    ...
    def _write_pets_record(self, vault: Path, pip_text: str, lana_text: str) -> None:
        write_markdown(
            ...
            {
                "type": "knowledge",
                "summary": "Pip is a tabby. Boots is black with a white spot on her chest.",
            },
            f"# Pets\n\n{pip_text}\n\n{lana_text}\n",
        )

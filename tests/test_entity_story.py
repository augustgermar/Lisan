...
class CoalesceTests(unittest.TestCase):
    ...
    def test_coalesce_key_by_entity_id(self) -> None:
        key = coalesce_key_for_job(
            ...
            {"entity_id": "entity.alice", "vault": str(self.vault)},
        )
        self.assertEqual(key, "entity.rewrite_story|entity_id=entity.alice")

    def test_coalesce_key_falls_back_to_entity_path(self) -> None:
        key = coalesce_key_for_job(
            "entity.rewrite_story",
            {"entity_path": "/vault/entities/people/alice.md"},
        )
        self.assertEqual(key, "entity.rewrite_story|entity_id=/vault/entities/people/alice.md")

    def test_five_enqueues_produce_one_job(self) -> None:
        payload = {
            "vault": str(self.vault),
            "entity_id": "entity.alice",
            "entity_path": str(self.vault / "entities" / "people" / "alice.md"),
        }
        for _ in range(5):
    ...
    def test_different_entities_do_not_coalesce(self) -> None:
        for name in ("entity.alice", "entity.bob"):
            enqueue_job(
                "entity.rewrite_story",
...
class RewriteEntityStoryTests(unittest.TestCase):
    ...
    def test_rewrite_updates_entity_body(self) -> None:
        entity_path = _seed_entity(self.vault, "alice", "Alice Smith", "Alice is a colleague.")

        # Draft episode with new material
        draft_path = self.vault / "drafts" / "test-draft.md"
        draft_path.parent.mkdir(parents=True, exist_ok=True)
        draft_body = "Alice and I worked on the project roadmap."
        ...
        new_narrative = "Alice Smith is a colleague of {{principal}}. They worked together on the project roadmap in June 2026."
        ...
        self.assertIn("Alice Smith", doc.body)
    ...
    def test_no_bypass_tokenize_principal_called(self) -> None:
        ...
        """
        entity_path = _seed_entity(self.vault, "alice", "Alice Smith")

        draft_path = self.vault / "drafts" / "draft2.md"
        draft_path.parent.mkdir(parents=True, exist_ok=True)
        draft_fm = {"id": "draft.test2", "type": "draft", "created": "2026-06-24", "updated": "2026-06-24"}
        draft_path.write_text(dump_markdown(draft_fm, "Alice talked to Jane about work."), encoding="utf-8")

        raw_narrative = "Alice Smith is a colleague of Jane."
        tokenized_narrative = "Alice Smith is a colleague of {{principal}}."

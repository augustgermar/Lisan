...
def new_schedule(
    ...
    if task_kind not in TASK_KINDS:
        raise ValueError(f"unknown task_kind {task_kind!r}; expected one of {sorted(TASK_KINDS)}")
    if not valid_cron(cron):
        raise ValueError(
            f"invalid cron {cron!r}; use every:<N><s|m|h|d|w>, daily@HH:MM, "
            "weekly:<mon..sun>@HH:MM, or monthly:<1-28>@HH:MM"
        )
    today = today_iso()
    safe_slug = slugify(title)
    path = vault / "schedules" / f"{today}-{safe_slug}.md"
    if path.exists():
        raise FileExistsError(path)
    frontmatter = {
        "id": f"schedule.{safe_slug}",
        "type": "schedule",
        "created": today,
        "updated": today,
        "status": "active",
        ...
        "links": links or [],
        "confidence": "high",
        "confidence_basis": "Owner-authored schedule definition",
        "last_confirmed": today,
        "review_after": today,
        "task_kind": task_kind,
        "cron": cron,
...
def new_confirmation(
    ...
    frontmatter = {
        ...
        "links": [task_id, *(links or [])],
        "confidence": "high",
        "confidence_basis": "Generated by the Adjutant gate from a CONFIRM verdict",
        "last_confirmed": today,
    ...
    body = (
        f"# {title}\n\n## What will happen\n\n{planned_action}\n\n"
        f"## Risk\n\n{risk}\n\n"
        f"Approve or deny before {expires}: `lisan confirm approve <id>` / `lisan confirm deny <id>`.\n"
    )
    write_markdown(path, with_domain_fields(frontmatter), body)
    return CreatedRecord(path=path, created=True)

...
def _format_item_detail(item: RetrievalItem, path: Path, lean: bool = False) -> str:
    ...
    if item.type == "knowledge":
        ...
        return _render([
            ...
            f"- source_tier: {fm.get('source_tier', 'unverified')}",
            None if lean else f"- source_origin: {fm.get('source_origin', 'unknown')}",
            None if lean else f"- source_url: {fm.get('source_url', 'unknown')}",
            None if lean else f"- retrieved_at: {fm.get('retrieved_at', 'unknown')}",

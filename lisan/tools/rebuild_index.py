...
def index_single_record(path: Path, vault: Path, conn: sqlite3.Connection) -> bool:
    ...
    row = {
        ...
        "source_document": str(fm.get("source_document", "")) if fm.get("source_document") is not None else None,
        "source_section": str(fm.get("source_section", "")) if fm.get("source_section") is not None else None,
        "source_ref": str(fm.get("source_ref", "")) if fm.get("source_ref") is not None else None,
        "source_url": str(fm.get("source_url", "")) if fm.get("source_url") is not None else None,
        "source_origin": str(fm.get("source_origin", "")) if fm.get("source_origin") is not None else None,
        "retrieved_at": str(fm.get("retrieved_at", "")) if fm.get("retrieved_at") is not None else None,
        "source_tier": str(fm.get("source_tier", "unverified")),
    ...
    conn.execute(
        ...
        INSERT OR REPLACE INTO files (
            ...
            source_document, source_section, source_ref, source_url, source_origin, retrieved_at, source_tier,
...
def _ensure_files_columns(conn: sqlite3.Connection) -> None:
    ...
    for column, decl in [
        ("source_document", "TEXT"), ("source_section", "TEXT"), ("source_ref", "TEXT"),
        ("source_url", "TEXT"), ("source_origin", "TEXT"), ("retrieved_at", "TEXT"), ("source_tier", "TEXT"),
        ("execute_asap", "INTEGER"),
        ("task_kind", "TEXT"),
        ("task_status", "TEXT"),
        ("next_run", "TEXT"),

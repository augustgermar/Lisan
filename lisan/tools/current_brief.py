...
def generate_current_brief(vault: Path | None = None) -> str:
    ...
    if stale_states:
        ...
        for state in stale_states:
            lines.append(f"- {state.arena}: {state.summary}")
        lines.append("")
...
def _load_states(vault: Path) -> list[BriefState]:
    ...
    for path in sorted((vault / "state").glob("*.md")):
        try:
            ...
            continue
        if str(doc.frontmatter.get("type")) != "state":
            continue
        arena = str(doc.frontmatter.get("arena_primary", ""))
        summary = str(doc.frontmatter.get("summary", ""))
        confidence = str(doc.frontmatter.get("confidence", "low"))
        ttl_days = doc.frontmatter.get("ttl_days")
        try:
            ttl_days_int = int(ttl_days) if ttl_days is not None and ttl_days != "" else None
        except (TypeError, ValueError):
            ttl_days_int = None
        review_after = str(doc.frontmatter.get("review_after", "")) or None
        updated = str(doc.frontmatter.get("updated", "")) or None

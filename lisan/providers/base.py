...
class MockClient(ProviderClient):
    ...
    def _response_for(self, *, agent: str, prompt: str, schema: dict[str, Any] | None) -> str:
        ...
        if agent == "listener":
            if any(marker in lowered for marker in ["/remember", "name is jordan", "daughter alice", "two cats", "pip", "lana"]):
                return json.dumps(
                    {
        ...
        if agent == "writer":
            ...
            if "daughter alice" in lowered:
                summary = "Jordan is here with his daughter Alice watching a YouTube video about mixing ice cream flavors."
            elif "two cats" in lowered or "pip" in lowered or "lana" in lowered:
                summary = "Jordan has two cats named Pip and Lana."

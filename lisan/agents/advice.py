...
def _fallback_answer(text: str) -> str:
    ...
    if any(term in lowered for term in ["tuna", "pasta", "mayo", "celery", "salad"]):
        return (
            "Yep, that works. Tuna pasta salad is a solid move. Mix the pasta with tuna, mayo, and chopped celery, "
            "then season it with salt, pepper, and a little acid if you have it, like lemon juice or vinegar."
        )
    return "Yep. Start with the simplest version first, then adjust seasoning and texture to taste."

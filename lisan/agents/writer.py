...
class WriterAgent(PromptAgent):
    ...
    def run_json(self, user_input: str, **kwargs: Any) -> Any:
        ...
        # WO-ADJUTANT step 7: with the execution layer enabled OR in
        # calibration (the soak: writers think about tasks, cycles stay
        # forcibly dry), open loops and decisions use the v2 prompts that
        # may attach task fields. v1 stays the default — a vault without
        # an Adjutant never asks the writer to think about taskings.
        adjutant_cfg = self.config.get("adjutant") or {}
        if task in ("open_loop", "decision") and (
            bool(adjutant_cfg.get("enabled", False)) or bool(adjutant_cfg.get("calibration", False))
        ):
